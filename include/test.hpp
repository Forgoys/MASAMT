...
int test_spatial_locality()
{
    ...
    for (int x = 2; x <= 1024; x *= 2) {
        for (int i = 0; i <= 10; i++) {
            double p0 = i * 0.1;
            std::cout << "x=" << x << ", 0:" << p0 * 100 << "% -> " << calcuSpatialLocality({{0, p0}, {x, 1 - p0}})
                      << std::endl;
        }
    }
    ...
    for (int x = 2; x <= 1024; x *= 2) {
        for (int i = 0; i <= 10; i++) {
            double p1 = i * 0.1;
            std::cout << "x=" << x << ", 1:" << p1 * 100 << "% -> " << calcuSpatialLocality({{1, p1}, {x, 1 - p1}})
                      << std::endl;
        }
    }
 ...
 * 计算每字节的访问次数
 */
double calcuAccessPerByte(const unsigned long long &N, const unsigned long long &size)
{
...
void test_adi_operator()
{
    ...
    for (const auto &func : adi.functions) {
        ...
        std::sort(sorted_vars.begin(), sorted_vars.end(),
                  [](const VariableInfo &a, const VariableInfo &b) { return a.access > b.access; });
...
void test_access_strategy_deduct_per_dataset(const std::string opName)
{
    std::string datasets[] = {"MINI", "SMALL", "STANDARD", "LARGE", "EXTRALARGE"};
    ...
    for (const auto &dataset : datasets) {
        ...
        // 读取ADI算子信息
        OperatorInfo adi;
        adi.getOperatorInfoFromCSV(opName, csvPath);
        for (const auto &func : adi.functions) {
            // if (func.name == "adi_kernel1" || func.name == "adi_kernel2") {
...
void test_access_strategy_deduct_per_function()
{
    // std::string datasets[] = {"MINI", "SMALL", "STANDARD", "LARGE", "EXTRALARGE"};

    // for (const auto &dataset : datasets) {
    std::string dataset = "STANDARD";
    std::cout << "\n========== 测试 " << dataset << " 数据集 ==========\n" << std::endl;
